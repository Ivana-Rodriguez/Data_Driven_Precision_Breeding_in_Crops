...
#####Loading data set#############
dataset_file="Potato_Suecia_Rodomiro_Ortiz_2023"
##load(paste0(dataset_file,".RData"))

Geno <- read.csv("C:/Users/Elías/Documents/Resultados y datos R_ Genomic Selection/Descarga de Datos Geno_Pheno/Geno % Pheno Potato Suecia Rodomiro Ortiz 2023/Geno_Potato_Suecia_Rodomiro_Ortiz_2023.csv")
Pheno <- read.csv("C:/Users/Elías/Documents/Resultados y datos R_ Genomic Selection/Descarga de Datos Geno_Pheno/Geno % Pheno Potato Suecia Rodomiro Ortiz 2023/Pheno_Potato_Suecia_Rodomiro_Ortiz_2023.csv")
...
Traits_to_evaluate=colnames(Pheno)[c(3:9)]
...
#Pheno <- Pheno %>% arrange(Loc, GID)    ###Revisando codigos anteriores me vi en la necesida de desactivar este y el codigo funciono
Pheno <- droplevels(Pheno)
# Select the unique lines both in pheno and geno and sort them
colnames(Geno) <- rownames(Geno)
final_geno_lines <- intersect(Pheno$GID, rownames(Geno)) %>% sort()
...
for (t in 1:length(Traits_to_evaluate)){
  ...
  for(i in seq_along(folds )) {
    ...
    ETA2=list(
      Lines=list(model="FIXED",X=XL), 
      K_GE=list(model="RKHS",K=K_GE)
    )
...
write_csv(Predictions_Final_traits, file.path(results_dir, "Predictions_Final_Potato_Suecia_Rodomiro_Ortiz_2023_NO_ENV.csv"))
write_csv(Sumary_all_traits, file.path(results_dir, "Summary_all_traits_Potato_Suecia_Rodomiro_Ortiz_2023_NO_ENV.csv"))
